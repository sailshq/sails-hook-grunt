...
module.exports = function logGruntAbortedError(consoleMsg, stackTrace, sails) {
  ...
  sails.log.error('Please fix it, then **restart Sails** to continue running tasks (e.g. watching for changes in assets)');
  ...
  sails.log.error();
  sails.log.error(' *-> Are "grunt" and related grunt task modules installed locally?  Run `npm install` if you\'re not sure.');
  sails.log.error();
  sails.log.error(' *-> You might have a malformed LESS, SASS, CoffeeScript file, etc.');
  sails.log.error();
